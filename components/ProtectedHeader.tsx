...
export default function ProtectedHeader({ user }: { user: AppUser }) {
  return (
    <header className="bg-white border-b border-[#DDE1EA] sticky top-0 z-50 h-14 flex items-center justify-between px-6">
      <div className="flex items-center gap-2.5">
        ...
        <div className="w-px h-6 bg-[#DDE1EA]" />
        <span className="text-sm font-bold text-brand-blue">Community Bus</span>
        <span className="badge-blue ml-1">{ROLE_LABELS[user.role] ?? user.role}</span>
      </div>
      <div className="flex items-center gap-3">
        {/* Notification bell — wired up per role */}
        <button
        ...
        <span className="text-xs text-[#5E6470]">{user.displayName}</span>
